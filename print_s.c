...
void			print_right_s(t_printf *mod, const char *s)
{
	...
	to_out = ft_strlen(s) < mod->precision ? ft_strlen(s) : mod->precision;
	if (mod->precision == -1)
		to_out = ft_strlen(s);
	if (mod->width != -1)
	{
		...
		{
			mod->width--;
			mod->zero ? ft_putchar('0') : ft_putchar(' ');
		}
	}
	while (to_out--)
		ft_putchar(*s++);
...
void			print_left_s(t_printf *mod, const char *s)
{
	...
	outputed = 0;
	if (mod->precision != -1)
		to_out = ft_strlen(s) > mod->precision ? mod->precision : ft_strlen(s);
	else
		to_out = ft_strlen(s);
	while (to_out--)
		ft_putchar(s[outputed++]);
	if (mod->width != -1)
	{
		mod->width -= outputed;
		while (mod->width > 0)
